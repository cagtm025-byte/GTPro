# --- PostgreSQL compatibility shim for existing SQLite code (drop-in) ---
# This block must appear before any other imports in the file.
import os
import re
import types

# Prefer DATABASE_URL; else fall back to individual PG env vars
...
if not _DSN:
    ...
    _DSN = f"postgresql://{user_enc}:{pwd_enc}@{host}:{port}/{db}"


# Import the real drivers
try:
    import psycopg2
    import psycopg2.extras
except Exception as e:
    raise RuntimeError("psycopg2 is required for PostgreSQL mode. Install with: pip install psycopg2-binary") from e

# We still import sqlite3 so the rest of your code can keep using it.
import sqlite3 as _real_sqlite3

# ---------- SQL translators ----------

# Replace "?" placeholders with "%s" (outside quotes) to match psycopg2 paramstyle
def _translate_placeholders(sql: str, params):
    ...
    while i < len(sql):
        ...
        if ch == '"' and not in_single:
            ...
            out.append(ch); i += 1; continue
        if ch == "?" and not in_single and not in_double and count < needed:
            out.append("%s")
            count += 1
            i += 1
            continue
...
# Basic DDL normalizations: AUTOINCREMENT, BLOB → BYTEA
...
# Emulate PRAGMA table_info(table)
def _pg_table_info(cur, table_name: str):
    q = """
    SELECT
      row_number() OVER ()::int AS cid,
      column_name AS name,
      data_type AS type,
      ...
      ) THEN 1 ELSE 0 END)::int AS pk
    FROM information_schema.columns c
    WHERE table_schema='public' AND table_name=%s
    ORDER BY ordinal_position;
    """
    cur.execute(q, (table_name, table_name))
    return [tuple(row.values()) if isinstance(row, dict) else row for row in cur.fetchall()]

# Emulate sqlite_master table checks
def _pg_has_table(cur, table_name: str) -> bool:
    cur.execute(
...
def _pg_find_table_like(cur, like_fragment: str):
    cur.execute(
        "SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND lower(table_name) LIKE lower(%s) LIMIT 1;",
        (like_fragment,)
    )
    row = cur.fetchone()
...
# --- replace your existing _PgCompatCursor with this version ---
class _PgCompatCursor:
    def __init__(self, pg_cursor, pg_conn):
        self._cur = pg_cursor
        self._conn = pg_conn
        self.lastrowid = None
        # pandas.read_sql_query requires this attribute to exist
    ...
    def execute(self, sql, params=None):
        ...
        # Handle: PRAGMA table_info(x)
        m = re.match(r"^\s*pragma\s+table_info\((?P<tbl>[a-zA-Z0-9_]+)\)\s*;?\s*$", s, flags=re.IGNORECASE)
        if m:
            rows = _pg_table_info(self._cur, m.group("tbl"))
            self._rows = rows
            self._rowidx = 0
            # DB-API style: list of 7-tuples (name, type_code, display_size, internal_size, precision, scale, null_ok)
            self.description = [(n, None, None, None, None, None, None)
                                for n in ("cid", "name", "type", "notnull", "dflt_value", "pk")]
            return self

        # Handle: SELECT ... FROM sqlite_master WHERE type='table' AND name='...'
        if "from sqlite_master" in s_low and "type='table'" in s_low:
            m_name = re.search(r"name\s*=\s*'([^']+)'", s_low)
            if m_name:
                exists = _pg_has_table(self._cur, m_name.group(1))
                self._rows = [("name",)] if exists else []
                self._rowidx = 0
                self.description = [("name", None, None, None, None, None, None)]
                return self
            # Handle: lower(name) like '%...%'
            m_like = re.search(r"lower\(name\)\s+like\s+'([^']+)'", s_low)
            if m_like:
                found = _pg_find_table_like(self._cur, m_like.group(1))
                self._rows = [(found,)] if found else []
                self._rowidx = 0
                self.description = [("name", None, None, None, None, None, None)]
                return self

        # No-op: PRAGMA foreign_keys=ON
        if s_low.startswith("pragma ") and "foreign_keys" in s_low:
            self._rows = []
            self._rowidx = 0
            self.description = []  # valid "no columns" state
            return self

        # SQLite-ism: INSERT OR REPLACE INTO customers (...) ...
        if s_low.startswith("insert or replace into customers"):
            cols_match = re.search(r"into\s+customers\s*\(([^)]+)\)\s*values\s*\(([^)]+)\)", s, flags=re.IGNORECASE)
            if not cols_match:
                raise ValueError("INSERT OR REPLACE for customers must specify column list.")
            cols = [c.strip() for c in cols_match.group(1).split(",")]
            set_list = ", ".join(f"{c}=EXCLUDED.{c}" for c in cols if c != "mobile")
            upsert_sql = (
                f"INSERT INTO customers ({', '.join(cols)}) "
                f"VALUES ({', '.join(['%s']*len(cols))}) "
                f"ON CONFLICT (mobile) DO UPDATE SET {set_list};"
            )
            self._cur.execute(upsert_sql, params)
            self.lastrowid = None
            self.description = []  # DML → no column metadata
            return self

        # DDL normalization (AUTOINCREMENT/BLOB)
        if s_low.startswith("create table") or s_low.startswith("alter table"):
            s = _normalize_ddl(s)

        # Translate "?" → "%s"
        s, params = _translate_placeholders(s, params)

        # Execute real SQL on psycopg2 cursor
        self._cur.execute(s, params)
        # IMPORTANT: set DB-API description from the underlying cursor so pandas can read column names
        self.description = getattr(self._cur, "description", None)

        # Support lastrowid for INSERTs assuming 'id' serial/identity primary key
        if s_low.startswith("insert into "):
            tbl_match = re.match(r"insert\s+into\s+([a-zA-Z0-9_]+)", s_low)
            if tbl_match:
                tbl = tbl_match.group(1)
                try:
                    self._cur.execute("SELECT currval(pg_get_serial_sequence(%s, 'id'));", (tbl,))
        ...
        if self.description is None:
            # Some statements (e.g., DDL/DML) legitimately have no description
            self.description = []
...
class _PgCompatConnection:
    ...
    def cursor(self):
        ...
        return _PgCompatCursor(real, self._conn)
    def commit(self):
        self._conn.commit()
    def rollback(self):
        self._conn.rollback()
...
def _pg_connect_compat(*args, **kwargs):
    # ignore sqlite filename & flags; we always connect to the configured PG database
    return _PgCompatConnection(_DSN)

# Monkey-patch sqlite3.connect so the rest of your code stays unchanged
_real_sqlite3.connect = _pg_connect_compat
...
for emoji, label in menu_options:
    ...
    # Modify to avoid page refresh by using pushState to update the URL without refreshing
    menu_html += (
        f"<a class='menu-link' href='{href}' target='_self' "
        f"onclick=\"event.preventDefault(); window.history.pushState(null, '', '{href}'); window.location.reload();\">"
...
# ---------------- Sales Register (Unified Tabs: Sales, Advances, Outstanding, Invoice History, Customer Master, TCS, SFT, GST, Cash, Payments Ledger) ----------------
    ...
    with tab1:
        ...
        if df_all.empty:
            ...
            if start_date is not None and end_date is not None:
                try:
                    df_sales = df_sales[
                        (df_sales["invoice_date"] >= start_date)
                        & (df_sales["invoice_date"] <= end_date)
                    ]
                except Exception:
                    pass  # Defensive fallback

            # ---- Build Invoice-level Category Register (without Making Charges) ----
            def _sum_categories_from_items(items_json_str):
                """Return dict of category sums from items_json (defensive)."""
                cats = {
                    "Gold Ornaments": 0.0,
                    "Silver Ornaments": 0.0,
                    "Diamond Ornaments": 0.0,
                    "Polish Charge": 0.0,
                }
                try:
                ...
                for it in items:
                    try:
                        cat = (it.get("category") or it.get("cat") or it.get("item_category") or "").strip()
                        amount = float(it.get("amount") or 0.0)
                    except Exception:
                        cat = ""
                        amount = 0.0
                    if cat and cat.lower().startswith("gold"):
                        cats["Gold Ornaments"] += amount
                    elif cat and cat.lower().startswith("silver"):
                        cats["Silver Ornaments"] += amount
                    elif cat and cat.lower().startswith("diamond"):
                        cats["Diamond Ornaments"] += amount
                    elif cat and ("polish" in cat.lower() or "polish charge" in cat.lower()):
                        cats["Polish Charge"] += amount
                    else:
                        desc = str(it.get("item_name") or it.get("description") or "")
                        if "polish" in desc.lower():
                            cats["Polish Charge"] += amount
                return cats

            invoice_register_rows = []
            for _, inv in df_sales.iterrows():
                cats_sum = _sum_categories_from_items(inv.get("items_json"))
                invoice_register_rows.append({
                    "invoice_no": inv.get("invoice_no"),
                    "invoice_date": inv.get("invoice_date"),
                    "customer_name": inv.get("customer_name"),
                    "customer_mobile": inv.get("customer_mobile"),
                    "tcs_rate": inv.get("tcs_rate"),
                    "TCS Amount": inv.get("tcs_amount"),
                    "GST Rate": inv.get("gst_rate"),
                    "GST Amount": inv.get("gst_amount"),
                    "Gold Ornaments": cats_sum["Gold Ornaments"],
                    "Silver Ornaments": cats_sum["Silver Ornaments"],
                    "Diamond Ornaments": cats_sum["Diamond Ornaments"],
                    "Polish Charge": cats_sum["Polish Charge"],
                    "Grand Total": inv.get("grand_total") or 0.0,
                })

            df_invoice_register = pd.DataFrame(invoice_register_rows)

            inv_cols = [
                c
                for c in [
                    "invoice_no", "invoice_date", "customer_name", "customer_mobile",
                    "customer_state", "customer_gstin", "customer_pan", "subtotal",
                    "gst_rate", "gst_amount", "tcs_rate", "tcs_amount", "grand_total",
                    "payment_mode", "payment_received", "payment_status", "categories",
                ]
            ...
            # --- Sub Tabs for 4 tables ---
            tab_cat, tab_inv, tab_item, tab_cust = st.tabs(
                ["🗂 Account Register", "📋 Sale Register", "📦 Item-wise Sale Details", "👥 Customer Register"]
            )

            with tab_cat:
                st.subheader("🗂 Account Register")
                if df_invoice_register.empty:
                    st.info("No category-level data available.")
                else:
                    num_cols = [
                        "TCS Amount", "GST Amount", "Gold Ornaments",
                        "Silver Ornaments", "Diamond Ornaments",
                        "Polish Charge", "Grand Total"
                    ]
                    for nc in num_cols:
                        if nc in df_invoice_register.columns:
                            df_invoice_register[nc] = pd.to_numeric(df_invoice_register[nc], errors="coerce").fillna(0.0)
                    st.dataframe(df_invoice_register.fillna(""), use_container_width=True, height=420)
                    st.download_button(
                        "⬇️ Download Category CSV",
                        data=df_invoice_register.to_csv(index=False).encode("utf-8"),
                        file_name="sales_register_invoice_categories.csv",
                        mime="text/csv",
                        use_container_width=True,
                    )

            with tab_inv:
                st.subheader("📋 Sale Register")
                if df_sales.empty:
                    st.info("No invoice-level data available.")
                else:
                    st.dataframe(df_sales[inv_cols].fillna(""), use_container_width=True, height=420)
                    st.download_button(
                        "⬇️ Download Invoice CSV",
                        data=df_sales[inv_cols].to_csv(index=False).encode("utf-8"),
                        file_name="sales_register_invoices.csv",
                        mime="text/csv",
                        use_container_width=True,
                    )

            with tab_item:
                st.subheader("📦 Item-wise Sale Register")
                if df_items.empty:
                    ...
                    if "invoice_date" in df_items.columns:
                        try:
                            ...
                            pass
                    st.dataframe(df_items.fillna(""), use_container_width=True, height=420)
                    st.download_button(
                        "⬇️ Download Item-wise CSV",
                        data=df_items.to_csv(index=False).encode("utf-8"),
                        file_name="sales_register_items.csv",
                        mime="text/csv",
                        use_container_width=True,
                    )
            ...
            with tab_cust:
                ...
                # Build the requested customer-register table: one row per invoice with specified columns
                cust_cols = [
                    "invoice_no", "invoice_date", "customer_name",
                    "customer_mobile", "customer_state", "customer_gstin", "customer_pan"
                ]
                # Keep only columns that actually exist in df_sales
                available_cols = [c for c in cust_cols if c in df_sales.columns]
                if not available_cols:
                    st.info("No customer data available in invoices.")
                else:
                    df_cust_reg = df_sales[available_cols].copy()
                    # Ensure invoice_date is date-typed
                    if "invoice_date" in df_cust_reg.columns:
                        try:
                            df_cust_reg["invoice_date"] = pd.to_datetime(df_cust_reg["invoice_date"], errors="coerce").dt.date
                        except Exception:
                            pass
                    st.dataframe(df_cust_reg.fillna(""), use_container_width=True, height=420)
                    st.download_button(
                        "⬇️ Download Customer Register CSV",
                        data=df_cust_reg.to_csv(index=False).encode("utf-8"),
                        file_name="sales_register_customer_register.csv",
                        mime="text/csv",
                        use_container_width=True,
                    )
    ...
    with tab9:
        ...
        # --- Prepare opening balance and daily aggregates ---
        try:
            conn = get_conn()
            df_open_coll = pd.read_sql_query(
                "SELECT COALESCE(SUM(amount),0) AS total_collected FROM payments WHERE lower(COALESCE(mode,''))='cash' AND date < ?",
                conn, params=(str(start_date),)
            )
            df_open_dep = pd.read_sql_query(
                "SELECT COALESCE(SUM(amount),0) AS total_deposited FROM cash_deposits WHERE deposit_date < ?",
                conn, params=(str(start_date),)
            )
            conn.close()
        ...
        try:
            ...
            df_depo = pd.read_sql_query("""
                SELECT deposit_date AS date, COALESCE(SUM(amount),0) AS deposited
                FROM cash_deposits WHERE deposit_date BETWEEN ? AND ?
        ...
        # --- Invoice-wise cash (prepared once, used in invoice tab) ---
        try:
            conn = get_conn()
            query = """
                SELECT p.invoice_no AS invoice_no,
                       p.date AS date,
                       COALESCE(i.customer_name, '') AS customer_name,
                       COALESCE(i.customer_mobile, '') AS customer_mobile,
                       p.amount AS amount,
                       p.mode AS mode
                FROM payments p
                LEFT JOIN invoices i ON p.invoice_no = i.invoice_no
                WHERE lower(coalesce(p.mode,''))='cash' AND p.date BETWEEN ? AND ?
                ORDER BY p.date, p.invoice_no
            """
            df_cash_inv = pd.read_sql_query(query, conn, params=(str(start_date), str(end_date)))
            conn.close()
        except Exception:
            # fallback: payments-only
            try:
                conn = get_conn()
                query2 = """
                    SELECT invoice_no, date, '' AS customer_name, '' AS customer_mobile, amount, mode
                    FROM payments
                    WHERE lower(coalesce(mode,''))='cash' AND date BETWEEN ? AND ?
                    ORDER BY date, invoice_no
                """
                df_cash_inv = pd.read_sql_query(query2, conn, params=(str(start_date), str(end_date)))
                conn.close()
            except Exception:
                df_cash_inv = pd.DataFrame(columns=["invoice_no","date","customer_name","customer_mobile","amount","mode"])

        # Normalize invoice-wise dates & ensure columns
        if not df_cash_inv.empty:
            if "date" in df_cash_inv.columns:
                try:
                    df_cash_inv["date"] = pd.to_datetime(df_cash_inv["date"], errors="coerce").dt.date
                except Exception:
                    pass
            wanted = ["invoice_no", "date", "customer_name", "customer_mobile", "amount", "mode"]
            for col in wanted:
                if col not in df_cash_inv.columns:
                    df_cash_inv[col] = None
            df_cash_inv = df_cash_inv[wanted]
            df_cash_inv["mode"] = df_cash_inv["mode"].astype(str).fillna("").apply(lambda x: "cash" if x.strip().lower() == "cash" else x)
            df_cash_inv["amount"] = pd.to_numeric(df_cash_inv["amount"], errors="coerce").fillna(0.0)

        # --- Sub-tabs: Daily summary and Invoice-wise cash ---
        tab_daily, tab_invoice = st.tabs(["📅 Daily Cash Summary", "🧾 Invoice-wise Cash Register"])

        with tab_daily:
            st.subheader("📅 Daily Cash Summary")
            ...
            st.download_button(
                ...
                data=display_df[["Date","Collected","Deposited","Cash on Hand"]].to_csv(index=False).encode("utf-8"),
                file_name="cash_register_daily_summary.csv",
                mime="text/csv",
                use_container_width=True,
            )

            st.markdown("---")
            st.subheader("🏦 Record Cash Deposit")
        ...
        with tab_invoice:
            st.subheader("🧾 Invoice-wise Cash Register")
            if df_cash_inv.empty:
                st.info("No invoice-wise cash payments found for the selected date range.")
            else:
                st.dataframe(df_cash_inv.fillna(""), height=420, use_container_width=True)
                st.download_button(
                    "⬇️ Download Invoice-wise Cash CSV",
                    data=df_cash_inv.to_csv(index=False).encode("utf-8"),
                    file_name="cash_register_invoice_wise.csv",
                    mime="text/csv",
                    use_container_width=True,
                )
...
# ---------------- Payments Ledger (Compact + Hidden Sections, CRM Theme) ----------------
    ...
    st.markdown(f"""
        <div style='display:flex;justify-content:space-between;align-items:center'>
            <div>
                <h4 style='margin:0;color:#ffffff'>💳 Payments Ledger</h4>
                <div style='color:#d1d5db;font-size:13px;margin-top:4px'>Transactions & reconciliation.</div>
            </div>
            <div style='text-align:right'>
    ...
    # --- Tabs: Full ledger / Edit payment / Quick add payment / Non-cash payments ---
    tab_full, tab_edit, tab_quick, tab_noncash = st.tabs(["📋 Full ledger", "✏️ Edit payment", "➕ Quick add payment", "💳 Non-cash payments"])
    ...
    with tab_full:
        ...
        # Filters (visible)
        f1, f2, f3 = st.columns([2, 1, 1])
        search = f1.text_input("Search name / mobile / invoice / mode", placeholder="Type to filter...", key="pl_search_full")
        min_amt = f2.number_input("Min amount (₹)", min_value=0.0, value=0.0, step=1.0, key="pl_min_amt_full")
        limit = f3.number_input("Rows to show", min_value=10, max_value=1000, value=200, step=10, key="pl_limit_full")
        ...
        if search and search.strip():
            ...
            df_display = df_display[
                df_display["customer_name"].astype(str).str.lower().str.contains(q, na=False)
                | df_display["customer_mobile"].astype(str).str.contains(q, na=False)
                | df_display["invoice_no"].astype(str).str.contains(q, na=False)
                | df_display["mode"].astype(str).str.lower().str.contains(q, na=False)
            ]
        if min_amt and min_amt > 0:
            df_display["amount_num"] = pd.to_numeric(df_display["amount"], errors="coerce").fillna(0)
        ...
        show_cols = [c for c in ["id", "date", "invoice_no", "customer_name", "customer_mobile", "amount", "mode", "note"] if c in df_display.columns]
        st.dataframe(df_display[show_cols].style.format({"amount": "₹{:,.2f}"}), height=450)

        a1, a2 = st.columns([1,1])
        if a1.button("Export CSV", key="pl_export_full"):
            csvb = df_display.to_csv(index=False).encode("utf-8")
            st.download_button("Download CSV", data=csvb, file_name="payments_ledger_full_export.csv", mime="text/csv")
        if a2.button("Refresh", key="pl_refresh_full"):
            st.experimental_rerun()

    # ---------------- Edit payment Tab ----------------
    with tab_edit:
        st.markdown("<div style='margin-bottom:6px;color:#d1d5db;font-size:13px'>Select a payment to edit — changes are saved to the payments table.</div>", unsafe_allow_html=True)
        options = []
        id_to_row = {}
        for _, r in df_pay.iterrows():
            pid = r.get("id")
    ...
    # ---------------- Quick add payment Tab ----------------
    with tab_quick:
        st.markdown("<div style='margin-bottom:6px;color:#d1d5db;font-size:13px'>Quickly add a payment. You can lookup invoices by name/mobile to prefill outstanding amount.</div>", unsafe_allow_html=True)
        a1, a2, a3, a4 = st.columns([1.2, 1, 1, 1])
        with a1:
            new_cust_name = st.text_input("Customer Name", key="pl_new_cust_name", placeholder="Customer full name")
        with a2:
        ...
        with a3:
            ...
            if prev_amt is None:
                ...
                # preserve previous typed value unless invoice just changed
                if st.session_state.get("pl_selected_invoice_label") != selected_invoice_label if 'selected_invoice_label' in locals() else False:
                    amt_value = default_amt
                else:
                    amt_value = prev_amt
            new_amt = st.number_input("Amount (₹)", min_value=0.0, step=0.01, value=float(amt_value), key="pl_new_amt")
    ...
    # ---------------- Non-cash payments Tab (new) ----------------
    with tab_noncash:
        st.markdown("<div style='margin-bottom:6px;color:#d1d5db;font-size:13px'>All payments where mode is not 'Cash'.</div>", unsafe_allow_html=True)

        # prepare filtered DF excluding cash (case-insensitive)
        df_nc = df_pay.copy()
        # normalize mode column and filter out 'cash' values
        df_nc["mode_clean"] = df_nc["mode"].astype(str).str.strip().str.lower()
        df_nc = df_nc[df_nc["mode_clean"] != "cash"]

        # select and order columns exactly as requested
        cols_wanted = ["invoice_no", "date", "customer_name", "customer_mobile", "amount", "mode"]
        cols_present = [c for c in cols_wanted if c in df_nc.columns]
        df_nc_display = df_nc[cols_present].fillna("")

        if df_nc_display.empty:
            st.info("No non-cash payments found.")
        else:
            # format amount column if present
            if "amount" in df_nc_display.columns:
                try:
                    df_nc_display["amount"] = pd.to_numeric(df_nc_display["amount"], errors="coerce").fillna(0.0)
                except Exception:
                    pass

            # show table and download
            # use style.format only if amount present
            if "amount" in df_nc_display.columns:
                st.dataframe(df_nc_display.style.format({"amount": "₹{:,.2f}"}), height=420)
            else:
                st.dataframe(df_nc_display, height=420)

            st.download_button("⬇️ Download Non-cash CSV", data=df_nc_display.to_csv(index=False).encode("utf-8"), file_name="non_cash_payments.csv", mime="text/csv")
        if st.button("Refresh Non-cash"):
            st.experimental_rerun()
